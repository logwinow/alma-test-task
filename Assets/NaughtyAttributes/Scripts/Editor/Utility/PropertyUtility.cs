...
namespace NaughtyAttributes.Editor
{
    public static class PropertyUtility
    {
        ...
        public static bool IsVisible(SerializedProperty property)
        {
            ...
            if (showIfAttribute.ComplexConditions?.Length > 0)
            {
                return GetComplexConditionsFlag(target, showIfAttribute.ComplexConditions, showIfAttribute.Inverted);
            }
        ...
        internal static bool GetComplexConditionsFlag(object target, (string Name, bool Value)[] conditions, bool invert)
        {
            var actualValues = GetConditionValues(target, conditions.Select(c => c.Name).ToArray());

            for (int i = 0; i < conditions.Length; i++)
            {
                if (actualValues[i] != conditions[i].Value)
                    return invert;
            }
